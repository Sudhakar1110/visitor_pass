...
6. `report.json` — the "Visitor Reconciliation" script report record
7. `dashboard_chart_source.json` — custom chart source for "Peak Visit Hours"
   (`source_name` is the dotted path to `utils.get_peak_visit_hours`)
8. `dashboard_chart.json` — the 4 dashboard charts
9. `number_card.json` — the 2 dashboard number cards
10. `dashboard.json` — the "Visitor Overview" dashboard
